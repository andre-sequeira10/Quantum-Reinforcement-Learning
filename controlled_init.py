...
class ctrl_Initialize(Instruction):
	...
	def __init__(self, params, ctrl_state, num_qubits=None):
		...
		if isinstance(params, str):
			...
			if self.num_qubits is None:
				self.num_qubits = int(math.log2(params)) + 1
			params = [params]
		else:
			self.num_qubits = math.log2(len(params))
		...
		super().__init__("ctrl_initialize", self.num_qubits, 0, params)
	...
	def gates_to_uncompute(self):
		...
		for i in range(self.num_qubits):
			...
			(remaining_param, thetas, phis) = ctrl_Initialize._rotations_to_disentangle(remaining_param)

			# perform the required rotations to decouple the LSB qubit (so that
			# it can be "factored" out, leaving a shorter amplitude vector to peel away)
			qregs = [q for q in cq] + [qubit for qubit in q[i:self.num_qubits]]
			add_last_cnot = True
			...
			if np.linalg.norm(thetas) != 0:
				...
				circuit.append(ry_mult.to_instruction().reverse_ops(), qregs)
	...
	def _rotations_to_disentangle(local_param):
		...
		for i in range(param_len // 2):
			...
			(remains, add_theta, add_phi) = ctrl_Initialize._bloch_angles(
	...
	def _multiplex(self, target_gate, list_of_angles, last_cnot=True):
		...
		qregs = [i for i in cq] + [i for i in q[0:-1]]
		if list_len > 1:
			circuit.append(multiplex_2.to_instruction().reverse_ops(), qregs)
		else:
...
def ctrl_initialize(self, statevector=None ,ctrl_state=None, ctrl_qubits=None, qubits=None):
	...
	num_qubits = None if not isinstance(statevector, int) else len(qubits)
